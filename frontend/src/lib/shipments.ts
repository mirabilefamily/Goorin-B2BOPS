...
export type Shipment = { id: string; order: Order; created: string; stage: number; coo: boolean; incoterms: string; transport: string; forwarder: string; prepaidAt: string };
...
export const stageLabel = (s: Shipment) => (s.stage >= 6 ? 'Invoiced' : s.stage === 5 ? 'Shipped' : 'Prepaid');
export const stageTone = (s: Shipment) => (s.stage >= 6 ? 'green' : s.stage === 5 ? 'teal' : 'blue');
